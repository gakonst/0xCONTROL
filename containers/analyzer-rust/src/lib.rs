...
pub struct AnalyzerService {
    presets: &'static HashMap<&'static str, Preset>,
...
impl AnalyzerService {
    ...
    pub fn router(&self) -> Router {
        let service = *self;
        let analyze = move |Query(query): Query<AnalyzeQuery>, body: Bytes| async move {
            service.handle_request(query, body).await.map(Json)
        };
    ...
    async fn handle_request(
        ...
        let pcm = self.decode_to_pcm(&body).await?;

        let waveform = self
            .analyze_waveform(&pcm, resolution, fft_size)
            .ok_or(AnalyzeError::Fft)?;
        let waveform = self.apply_preset(&waveform, preset);
        let (bpm, beat_offset_seconds) = self.estimate_bpm_and_offset(&pcm);
...
impl AnalyzerService {
    async fn decode_to_pcm(&self, input: &[u8]) -> Result<PcmData, AnalyzeError> {
        ...
        if let Some(mut stdin) = child.stdin.take() {
            stdin.write_all(input).await.map_err(|err| {
                AnalyzeError::Internal(format!("failed to write to ffmpeg: {err}"))
            })?;
        }
    ...
    fn analyze_waveform(
        ...
        let mono = Self::mix_to_mono(&pcm.samples);
        if mono.is_empty() {
            return None;
        }

        let hop = std::cmp::max(1, mono.len() / resolution);
        let hann = Self::build_hann_window(fft_size);
        ...
        for bar_index in 0..resolution {
            ...
            let energies = Self::accumulate_band_energy(&buffer, pcm.sample_rate, fft_size as u32);
        ...
        let bars: Vec<WaveformBar> = bands_per_bar
            ...
            .map(|band| {
                ...
                let voice = Self::normalize(
                    *band.get("voice").unwrap_or(&0.0) * voice_weight,
                    *maxima.get("voice").unwrap_or(&1.0) * voice_weight,
                    0.6,
                );
                let melody = Self::normalize(
                    *band.get("melody").unwrap_or(&0.0) * melody_weight,
                    *maxima.get("melody").unwrap_or(&1.0) * melody_weight,
                    0.5,
                );
                let air = Self::normalize(
                    *band.get("air").unwrap_or(&0.0),
                    *maxima.get("air").unwrap_or(&1.0),
                    0.9,
                );

                let whiteness = Self::clamp(air.powf(1.05) * 0.2, 0.0, 0.22);
                let r = Self::clamp(bass * 1.18 * (1.0 - whiteness) + whiteness * 0.8, 0.0, 1.0);
                let g = Self::clamp(voice * 1.22 * (1.0 - whiteness) + whiteness * 0.8, 0.0, 1.0);
                let b = Self::clamp(melody * 1.38 * (1.0 - whiteness) + whiteness, 0.0, 1.0);
    ...
    fn apply_preset(&self, waveform: &WaveformData, preset: &Preset) -> WaveformData {
        ...
        let mut bars: Vec<WaveformBar> = waveform
            ...
            .map(|bar| {
                ...
                if preset.saturation_boost != 0.0 || preset.brightness_boost != 0.0 {
                    ...
                    let s2 = Self::clamp01(s * (1.0 + preset.saturation_boost));
                    let l2 = Self::clamp01(l * (1.0 + preset.brightness_boost));
                    let rgb = Self::hsl_to_rgb(h, s2, l2);
                    r = rgb.r;
                    g = rgb.g;
                    b = rgb.b;
                }

                r = Self::clamp01(r * (1.0 - whiteness) + whiteness);
                g = Self::clamp01(g * (1.0 - whiteness) + whiteness);
                b = Self::clamp01(b * (1.0 - whiteness) + whiteness);
    ...
    fn normalize(value: f32, max: f32, gamma: f32) -> f32 {
        ...
        let ratio = Self::clamp(value / max, 0.0, 1.0);
        ratio.powf(gamma)
    }
    ...
    fn estimate_bpm_and_offset(&self, pcm: &PcmData) -> (Option<f32>, Option<f32>) {
        let min_bpm = 70.0;
        let max_bpm = 180.0;
        ...
        let max_lag = ((envelope_rate * 60.0 / min_bpm).round() as usize)
            .min(envelope.len().saturating_sub(2));
        if max_lag <= min_lag {
            return (None, None);
        }
...
mod tests {
    ...
    fn hann_window_has_expected_shape() {
        let window = AnalyzerService::build_hann_window(5);
    ...
    fn normalize_handles_zero_max_and_gamma() {
        approx_eq(AnalyzerService::normalize(10.0, 0.0, 0.5), 0.0, 1e-6);
        approx_eq(AnalyzerService::normalize(2.0, 4.0, 1.0), 0.5, 1e-6);
        approx_eq(
            AnalyzerService::normalize(2.0, 8.0, 0.5),
            (0.25f32).sqrt(),
            1e-6,
        );
    }
    ...
    fn rgb_round_trip_preserves_color() {
        ...
        let (h, s, l) = AnalyzerService::rgb_to_hsl(r, g, b);
        let rgb = AnalyzerService::hsl_to_rgb(h, s, l);
    ...
    fn accumulate_band_energy_distributes_per_band() {
        ...
        let energy = AnalyzerService::accumulate_band_energy(&spectrum, sample_rate, fft_size);
    ...
    fn apply_preset_respects_caps_and_smoothing() {
        ...
        let service = AnalyzerService::new();
        let result = service.apply_preset(&waveform, &preset);
    ...
    fn estimate_bpm_detects_click_track() {
        ...
        let service = AnalyzerService::new();
        let (bpm, offset) = service.estimate_bpm_and_offset(&pcm);
